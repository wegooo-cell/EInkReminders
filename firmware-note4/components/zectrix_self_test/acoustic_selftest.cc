...
void PlaybackTask(void* arg) {
    std::unique_ptr<PlaybackContext> ctx(static_cast<PlaybackContext*>(arg));
...
AcousticSelftestRoundResult RunRound(AudioCodec* codec,
    ...
    // Ownership passed to the playback task, which frees it when it exits.
    playback_ctx.release();
...
AcousticSelftestSummary AcousticSelftest::Run(AudioCodec* codec, const std::array<uint8_t, 6>& mac) const {
    ...
    // valid() must be checked first: a codec whose chip failed to initialize
    // still reports the sample rates and channel counts it was constructed
    // with, so the checks below cannot detect it on their own.
    if (codec == nullptr || !codec->valid() || !codec->duplex() || codec->input_channels() != 1 ||
