...
// Field entry is non-destructive, so a short debounce is sufficient.
constexpr TickType_t kFieldEnterDebounce = pdMS_TO_TICKS(20);
// Field exit needs stronger filtering: a false exit can start I2C traffic
...
esp_err_t ZectrixNfc::BeginTransferSessionLocked(const char* reason) {
    ...
    // Do not power-cycle here: that would interrupt an active phone RF write.
    // The chip was powered during Init/PowerOn; only keep the I2C rail active.
    BoardI2cForcePowerOn();
...
void ZectrixNfc::FieldTask() {
    while (true) {
        ...
        // Confirm field exit with multiple samples. RF modulation or small
        // phone movements can briefly deassert FD during a write; treating a
        // single sample as exit could start I2C traffic and interrupt it.
        if (previously_present && !field_present) {
            for (int i = 1; i < kFieldExitConfirmSamples; ++i) {
                vTaskDelay(kFieldExitConfirmInterval);
                if (IsPowered() && IsFieldLevelActive()) {
                    field_present = true;
                    break;
                }
            }
