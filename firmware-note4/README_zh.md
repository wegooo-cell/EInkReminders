...
- 小范围画面变化采用 1bpp 局部刷新，累计 8 次局部刷新后执行一次全屏刷新。
- 使用 NOTE4 专用的局域网同步 API；Mac 应用会校验 400 × 300 黑白显示格式。
- 首次使用或在设置中主动选择“重新配网”时建立 `EInk-Note4-XXXX` 热点，在屏幕显示
  适配 iPhone 的 Wi-Fi 二维码，并通过系统配网门户提供完整中文流程。
- 已保存的 Wi-Fi 暂时断开时保留当前待办画面，并每 10 秒在后台自动重连，不会误回到
  扫码配网页面。扫码页可长按上键进入设置，再选择“重连 Wi-Fi”或“重新配网”。
- 离线完成操作保存在 NVS，断电不丢失。
